  ...
  <div class="container-fluid m-0 p-0 ">
    <div id="trains-table" class="full-height p-5">
      <div class="row pb-5 h-100 d-flex align-items-center justify-content-center">
        <h1 class="text-center mb-5 pt-5 pb-0">Edytuj pociąg:</h1>
                  ...
                  <td>
                    ...
                    <div class="form-group m-2">
                      <div class="form-group">
                        <input type="text" id="additionalInfoLocomotive" v-model="locomotive.additionalInfo"
                               style="width: 400px"/>
                      </div>
                    </div>
                  </td>
...
export default {
  ...
  created() {
    this.getTrainData(this.trainId).then(trainData => {
      ...
      this.route = trainData.route;
      this.runningDates = trainData.runningDates;
      this.additionalInfo = trainData.additionalInfo;
      this.selectedCars = trainData.selectedCars;
      this.selectedLocomotives = trainData.selectedLocomotives;
    });
    this.fetchCars();
  ...
  methods: {
    getTrainData(trainId) {
      return fetch(`http://localhost:3081/api/trains/get/${trainId}`)
          .then(response => {
            if (!response.ok) {
              throw new Error(`Failed to fetch train with id: ${trainId}`);
            }
    ...
    handleFormSubmit(editTrainData) {
      fetch('http://localhost:3081/api/trains/edit', {
        ...
        body: JSON.stringify(editTrainData)
      })
          .then(res => res.json())
          .then(() => {
